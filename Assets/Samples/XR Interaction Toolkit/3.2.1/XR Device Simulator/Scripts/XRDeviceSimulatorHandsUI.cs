...
namespace UnityEngine.XR.Interaction.Toolkit.Samples.DeviceSimulator
{
    ...
    internal class XRDeviceSimulatorHandsUI : MonoBehaviour
    {
        [Header("General")] [SerializeField] private Image m_HandImage;

        [SerializeField] private Sprite m_HandDefaultSprite;

        [SerializeField] private List<HandExpressionUI> m_Expressions = new();

        private HandExpressionUI m_ActiveExpression;
        ...
        internal void Initialize(XRDeviceSimulator simulator)
        {
            ...
            {
                Debug.LogError(
                    $"Could not find SimulatedHandExpressionManager component on {simulator.name}, disabling simulator UI.");
            ...
            for (var index = 0; index < m_HandExpressionManager.simulatedHandExpressions.Count; ++index)
            {
                ...
                else
                {
                    ...
                    switch (simulatedExpression.toggleInput.inputSourceMode)
                    {
                        ...
                        default:
                            Debug.LogWarning(
                                $"Toggle Input for Simulated Hand Expression \"{simulatedExpression.name}\" is not an input action, cannot display binding or icon in UI.",
                                this);
                            break;
                    }
        ...
        {
            foreach (var expression in m_Expressions) expression.UpdateButtonVisuals(active, m_MainUIManager);
            ...
            if (active)
            {
                foreach (var expression in m_Expressions)
                {
                    ...
                    expression.SetButtonColor(isActiveExpression
                        ? m_MainUIManager.selectedColor
                        : m_MainUIManager.buttonColor);
                }
            ...
            else
            {
                ...
                foreach (var expression in m_Expressions)
                {
                    ...
                    expression.SetButtonColor(isActiveExpression
                        ...
                        : m_MainUIManager.disabledButtonColor);
        ...
        internal void ToggleExpression(SimulatedHandExpression simulatedExpression, XRDeviceSimulator simulator)
        {
            ...
            if (index >= m_Expressions.Count)
                ...
                Debug.LogError(
                    $"The Device Simulator tried to toggle {simulatedExpression.name} but it was not found in the list of simulated hand expressions, the UI can not be updated.",
                    this);
            else
                ToggleExpression(m_Expressions[index]);
        }

        private void ToggleExpression(HandExpressionUI expression)
        {
        ...
        private void SetExpressionActiveStatus(bool isActive, HandExpressionUI expression)
        {
        ...
        [Serializable]
        private class HandExpressionUI
        {
            ...
            [SerializeField] private Text m_BindText;
